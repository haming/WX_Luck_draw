...
router.get("/index_select_Users_top_ten",function(req,res){
    ...
    dao.selectUsers_all_top_ten(function(err,result){
        ...
        if(result === false || result == "false" || result == null || result == ""){
            console.log("没有数据!");
            res.send("nolottery");
            res.end();
        }else {
 ...
 * 取出所有奖品信息
 */
router.get('/selectlottery', function (req, res) {
    dao.admin_select_all_gifts(function(err,result){
        res.send(result);
    });
});
 ...
 *奖品数量处理
 */
router.get('/UpdateGift_number', function (req, res) {
    var giftnums = req.query.giftnums;
    var giftid = req.query.giftid;
    dao.gifts_manager(giftnums,giftid,function(err,result){
        if (err == null){
            res.send("000");
        }else{
            res.send("001");
